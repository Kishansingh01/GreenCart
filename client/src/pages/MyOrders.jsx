...
const MyOrders=()=>{
  ...
  const fetchMyOrders = async () => {
    try {
        // Safely grab the ID (handling both _id and id formats)
        const currentUserId = user._id || user.id;

        // ✅ FIX 2: Attach the userId to the URL as a query parameter
        const { data } = await axios.get(`/api/order/user?userId=${currentUserId}`);
